...
- **What does the commit log reveal about groupings?** Commits are the author's own statement of what belonged together. A commit like `chore: bootstrap agent skills` tells you that a batch of structurally diverse files were one intentional unit — even if they'd otherwise scatter across several path-based buckets. A commit like `chore: regen CRD manifests` tells you a large YAML change is mechanical, not hand-written. Use this to:
